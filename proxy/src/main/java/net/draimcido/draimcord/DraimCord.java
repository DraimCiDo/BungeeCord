...
import net.draimcido.draimcord.captcha.CaptchaGeneration;
import net.draimcido.draimcord.config.Settings;
import net.draimcido.draimcord.utils.GeoIp;
import net.draimcido.draimcord.utils.ManyChecksUtils;
import net.draimcido.draimcord.utils.ServerPingUtils;
import net.draimcido.draimcord.utils.Sql;
...
public class DraimCord
{
    ...
    //UserName, Ip
    @Getter
    private final Map<String, DraimCordUser> userCache = new ConcurrentHashMap<>();
    ...
    public DraimCord(boolean startup) {
        ...
        DraimCordThread.start();
    }

    public void disable() {
        DraimCordThread.stop();
    ...
    public void saveUser(String userName, InetAddress address, boolean afterCheck) {
        ...
        DraimCordUser draimCordUser = userCache.get( userName );
        if ( draimCordUser == null )
        {
            draimCordUser = new DraimCordUser( userName, address.getHostAddress(), timestamp, timestamp );
        } else
        {
            draimCordUser.setIp( address.getHostAddress() );
            draimCordUser.setLastJoin( timestamp );
            if ( afterCheck )
            {
                draimCordUser.setLastCheck( timestamp );
            }
        }

        userCache.put( userName, draimCordUser );
        if ( sql != null )
        {
            sql.saveUser( draimCordUser );
        }
    }

    public void addUserToCache(DraimCordUser draimCordUser)
    {
        userCache.put( draimCordUser.getName(), draimCordUser );
    }
    ...
    public void connectToDraimCord(UserConnection userConnection)
    {
        userConnection.getCh().setEncoderProtocol( Protocol.GAME );
        userConnection.getCh().setDecoderProtocol( Protocol.DraimCord );
    ...
    public boolean needCheck(String userName, InetAddress address) {
        DraimCordUser draimCordUser = userCache.get( userName.toLowerCase() );
        if ( Settings.IMP.PROTECTION.CHECK_LOCALHOST >= 1 && address.isLoopbackAddress() )
        {
            return Settings.IMP.PROTECTION.CHECK_LOCALHOST != 1;
        }
        return draimCordUser == null || ( Settings.IMP.FORCE_CHECK_ON_ATTACK && isUnderAttack() )
                || !draimCordUser.getIp().equalsIgnoreCase( address.getHostAddress() );
    }
