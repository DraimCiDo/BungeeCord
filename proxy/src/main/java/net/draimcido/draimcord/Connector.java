...
import static net.draimcido.draimcord.DraimCord.CheckState.*;
import static net.draimcido.draimcord.caching.PacketUtils.*;
...
public class Connector extends MoveHandler
{
    ...
    @Getter
    @Setter
    private DraimCord.CheckState state = DraimCord.CheckState.CAPTCHA_ON_POSITION_FAILED;
    @Getter
    ...
    public Connector(UserConnection userConnection, DraimCord draimCord)
    {
        this.draimCord = draimCord;
        this.state = this.draimCord.getCurrentCheckState();
        ...
        if ( Settings.IMP.PROTECTION.SKIP_GEYSER && draimCord.isGeyser( userConnection.getPendingConnection() ) )
        {
            this.state = ONLY_CAPTCHA;
        }
    }
    ...
    public void spawn()
    {
        ...
        {
            PacketUtils.spawnPlayer( channel, userConnection.getPendingConnection().getVersion(), state == DraimCord.CheckState.ONLY_CAPTCHA, true );
            sendCaptcha();
    ...
    public void exception(Throwable t) throws Exception
    {
        ...
        if ( state == DraimCord.CheckState.FAILED )
        {
            channel.close();
        } else
        {
    ...
    public void disconnected(ChannelWrapper channel) throws Exception
    {
        ...
        draimCord.removeConnection( null, this );
        disconnected();
    }
    ...
    {
        if ( System.currentTimeMillis() - joinTime < TOTAL_TIME && state != DraimCord.CheckState.ONLY_CAPTCHA )
        {
            if ( state == DraimCord.CheckState.CAPTCHA_POSITION && aticks < TOTAL_TICKS )
            {
                channel.writeAndFlush( PacketUtils.getCachedPacket( PacketsPosition.SETSLOT_RESET ).get( version ), channel.voidPromise() );
                state = DraimCord.CheckState.ONLY_POSITION;
            } else
            {
                if ( state == DraimCord.CheckState.CAPTCHA_ON_POSITION_FAILED )
                {
        ...
        if ( draimCord.checkBigPing( totalping / ( devide <= 0 ? 1 : devide ) ) )
        {
            failed( PacketUtils.KickType.PING, "Big ping" );
            return;
        }
        state = DraimCord.CheckState.SUCCESSFULLY;
        PacketUtils.titles[2].writeTitle( channel, version );
        channel.flush();
        draimCord.removeConnection( null, this );
        sendMessage( PacketsPosition.CHECK_SUS_MSG );
        draimCord.saveUser( getName(), IPUtils.getAddress( userConnection ), true );
        ...
        markDisconnected = true;
        LOGGER.log( Level.INFO, "[DraimCord] Player (" + name + "|" + ip + ") successfully passed verification." );
    }

    @Override
    public void onMove()
    {
        if ( lastY == -1 || state == DraimCord.CheckState.FAILED || state == DraimCord.CheckState.SUCCESSFULLY || onGround )
        {
            return;
        }
        if ( state == DraimCord.CheckState.ONLY_CAPTCHA )
        {
        ...
        if ( y <= 60 && state == DraimCord.CheckState.CAPTCHA_POSITION && waitingTeleportId == -1 )
        {
            resetPosition( false );
        }
        if ( aticks >= TOTAL_TICKS && state != DraimCord.CheckState.CAPTCHA_POSITION )
        {
            completeCheck();
            return;
        }
        if ( state == DraimCord.CheckState.CAPTCHA_ON_POSITION_FAILED || state == DraimCord.CheckState.ONLY_POSITION )
        {
    ...
    {
        if ( this.lastSend == 0 && !( state == DraimCord.CheckState.FAILED || state == DraimCord.CheckState.SUCCESSFULLY ) )
        {
    ...
    {
        state = DraimCord.CheckState.ONLY_CAPTCHA;
    ...
    {
        state = DraimCord.CheckState.FAILED;
